# lambdacomparator
(in progress) Like the 15 param occamsfuncer universalFunc (which is designed to simple and human intuitive) but is only 6 params and designed to be as simple as possible. Needs a recursive equality edge, where x==y if x.L==y.L and x.R==y.R, to use func-param-return-caching anonymously (without ids of nodes, such as in occamsmanifold).
